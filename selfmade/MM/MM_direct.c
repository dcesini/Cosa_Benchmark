  ...
  /*** Display who does which iterations for demonstration purposes ***/
	...
	if (tid == 0)
	   ...
	   printf("time for C(%d,%d) = A(%d,%d) B(%d,%d) is %llu ns = %fs \n", dim1, dim3, dim1, dim2, dim2, dim3, (long long unsigned int)timeElapsed, timesec);	
        ...
        /* Compute C = A B with dgemm */
      printf("Compute with DGEMM\n");
      ...
      if(NO_WRITE) {
        printf("Writing M1*M2 to file M1M2_DGEMM.dat\n");
        fp = fopen("M1M2_DGEMM.dat", "w");
        if (fp == NULL) {
           printf("I couldn't open M1M2_DGEMM.dat for writing.\n");
           exit(0);
        }
        for (i = 0; i < dim2; i++)
           for (j = 0; j < dim3; j++)
